...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<CarDriveStamped>)))
  ...
  (cl:format cl:nil "# CarDrive message with timestamp.~%# $Id$~%~%Header    header~%CarDrive  control~%~%================================================================================~%MSG: std_msgs/Header~%# Standard metadata for higher-level stamped data types.~%# This is generally used to communicate timestamped data ~%# in a particular coordinate frame.~%# ~%# sequence ID: consecutively increasing ID ~%uint32 seq~%#Two-integer timestamp that is expressed as:~%# * stamp.sec: seconds (stamp_secs) since epoch (in Python the variable is called 'secs')~%# * stamp.nsec: nanoseconds since stamp_secs (in Python the variable is called 'nsecs')~%# time-handling sugar is provided by the client library~%time stamp~%#Frame this data is associated with~%# 0: no frame~%# 1: global frame~%string frame_id~%~%================================================================================~%MSG: art_msgs/CarDrive~%# Driving command for a car-like vehicle using Ackermann steering.~%# $Id$~%~%# Drive at requested speed, acceleration and jerk (the 1st, 2nd and~%# 3rd derivatives of position). All are non-negative scalars. ~%#~%# Speed is defined as the scalar magnitude of the velocity~%# vector. Direction (forwards or backwards) is determined by the gear.~%#~%# Zero acceleration means change speed as quickly as~%# possible. Positive acceleration may include deceleration as needed~%# to match the desired speed. It represents a desired rate and also a~%# limit not to exceed.~%#~%# Zero jerk means change acceleration as quickly as possible. Positive~%# jerk describes the desired rate of acceleration change in both~%# directions (positive and negative).~%#~%float32 speed                   # magnitude of velocity vector (m/s)~%float32 acceleration            # desired acceleration (m/s^2)~%float32 jerk                    # desired jerk (m/s^3)~%~%# Assumes Ackermann (front-wheel) steering. This angle is the average~%# yaw of the two front wheels in the vehicle frame of reference~%# (positive left), ignoring their slightly differing angles as if it~%# were a tricycle. This is *not* the angle of the steering wheel~%# inside the passenger compartment.~%#~%float32 steering_angle          # steering angle (radians)~%~%Gear gear                       # requested gear (no change if Naught)~%PilotBehavior behavior          # requested pilot behavior~%~%================================================================================~%MSG: art_msgs/Gear~%# ART vehicle transmission gear numbers~%#~%# Used by several different messages.~%~%# $Id$~%~%# Gear numbers. ~%#~%# Naught means: reset all Shifter relays; no change of CarDrive gear.~%uint8 Naught = 0~%uint8 Park = 1~%uint8 Reverse = 2~%uint8 Neutral = 3~%uint8 Drive = 4~%uint8 N_gears = 5~%~%uint8 value		# requested or reported gear number~%~%================================================================================~%MSG: art_msgs/PilotBehavior~%# ART autonomous vehicle pilot node behaviors.~%#~%# Normally, the pilot node does Run, continually sending commands to~%# the servo device actuators and monitoring their state.  With Pause,~%# the pilot becomes passive, allowing a learning algorithm or human~%# controller direct access to those devices.  In the Off state,~%# various devices are shut down: the transmission in Park, the brake~%# released, the throttle at idle.  The engine is not turned off, but~%# it could be.~%~%# $Id$~%~%# Behavior value~%uint8 value~%~%# Behavior numbers:~%uint8 Run = 0                   # normal driving~%uint8 Pause = 1                 # stop issuing servo commands~%uint8 Off = 2                   # turn off devices~%uint8 N_behaviors = 3~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'CarDriveStamped)))
  "Returns full string definition for message of type 'CarDriveStamped"
  (cl:format cl:nil "# CarDrive message with timestamp.~%# $Id$~%~%Header    header~%CarDrive  control~%~%================================================================================~%MSG: std_msgs/Header~%# Standard metadata for higher-level stamped data types.~%# This is generally used to communicate timestamped data ~%# in a particular coordinate frame.~%# ~%# sequence ID: consecutively increasing ID ~%uint32 seq~%#Two-integer timestamp that is expressed as:~%# * stamp.sec: seconds (stamp_secs) since epoch (in Python the variable is called 'secs')~%# * stamp.nsec: nanoseconds since stamp_secs (in Python the variable is called 'nsecs')~%# time-handling sugar is provided by the client library~%time stamp~%#Frame this data is associated with~%# 0: no frame~%# 1: global frame~%string frame_id~%~%================================================================================~%MSG: art_msgs/CarDrive~%# Driving command for a car-like vehicle using Ackermann steering.~%# $Id$~%~%# Drive at requested speed, acceleration and jerk (the 1st, 2nd and~%# 3rd derivatives of position). All are non-negative scalars. ~%#~%# Speed is defined as the scalar magnitude of the velocity~%# vector. Direction (forwards or backwards) is determined by the gear.~%#~%# Zero acceleration means change speed as quickly as~%# possible. Positive acceleration may include deceleration as needed~%# to match the desired speed. It represents a desired rate and also a~%# limit not to exceed.~%#~%# Zero jerk means change acceleration as quickly as possible. Positive~%# jerk describes the desired rate of acceleration change in both~%# directions (positive and negative).~%#~%float32 speed                   # magnitude of velocity vector (m/s)~%float32 acceleration            # desired acceleration (m/s^2)~%float32 jerk                    # desired jerk (m/s^3)~%~%# Assumes Ackermann (front-wheel) steering. This angle is the average~%# yaw of the two front wheels in the vehicle frame of reference~%# (positive left), ignoring their slightly differing angles as if it~%# were a tricycle. This is *not* the angle of the steering wheel~%# inside the passenger compartment.~%#~%float32 steering_angle          # steering angle (radians)~%~%Gear gear                       # requested gear (no change if Naught)~%PilotBehavior behavior          # requested pilot behavior~%~%================================================================================~%MSG: art_msgs/Gear~%# ART vehicle transmission gear numbers~%#~%# Used by several different messages.~%~%# $Id$~%~%# Gear numbers. ~%#~%# Naught means: reset all Shifter relays; no change of CarDrive gear.~%uint8 Naught = 0~%uint8 Park = 1~%uint8 Reverse = 2~%uint8 Neutral = 3~%uint8 Drive = 4~%uint8 N_gears = 5~%~%uint8 value		# requested or reported gear number~%~%================================================================================~%MSG: art_msgs/PilotBehavior~%# ART autonomous vehicle pilot node behaviors.~%#~%# Normally, the pilot node does Run, continually sending commands to~%# the servo device actuators and monitoring their state.  With Pause,~%# the pilot becomes passive, allowing a learning algorithm or human~%# controller direct access to those devices.  In the Off state,~%# various devices are shut down: the transmission in Park, the brake~%# released, the throttle at idle.  The engine is not turned off, but~%# it could be.~%~%# $Id$~%~%# Behavior value~%uint8 value~%~%# Behavior numbers:~%uint8 Run = 0                   # normal driving~%uint8 Pause = 1                 # stop issuing servo commands~%uint8 Off = 2                   # turn off devices~%uint8 N_behaviors = 3~%~%~%"))
