...
#if 0
  if (value) {
    ROS_DEBUG("Point in small poly: original poly: [%f,%f; %f,%f;%f,%f;%f,%f]"
			       ...
			       float x, float y)
{
  ...
  if (pindex < 0)
    {
      ROS_DEBUG("no polygon contains point (%.3f, %.3f)", x, y);
      return -1;			// no match
    }
...
#define EXTREME_DEBUG 1
#ifdef EXTREME_DEBUG
	    ROS_DEBUG("adding start, end waypoints %s, %s, poly_id = %d",
				  ...
				  std::vector <poly> &to_polys, ElementID id)
{
  ...
  ROS_DEBUG("scanning polygons from %d back to 0", last);
...
#ifdef EXTREME_DEBUG
  ROS_DEBUG("distance_total to (%f, %f) %f )", to.x, to.y, distance_total);
  ROS_DEBUG("polygon_length to (%f, %f) %f )", to.x, to.y, polygon_length);
#endif
				    ...
				    const float poly_epsilon) 
{
  ...
  if (cur_poly_index == -1) {
    ROS_DEBUG("PolyOps::getPolyLaneIdDir: No poly found");
  ...
  ROS_DEBUG("Error: lane must not exist this direction");
				    ...
				    const float poly_epsilon) 
{
  ...
  if (cur_poly_index == -1) {
    ROS_DEBUG("PolyOps::getNonTransPolyLaneIdDir: No poly found");
    return ElementID(-1,-1,-1);
  }
                         ...
                         const MapPose &pose) 
{
  ...
  if (cur_poly_index == -1) {
    ROS_DEBUG("PolyOps::getLaneDir: No poly found");
    return;
  }
  int adj_poly_index[2];
  poly my_cur_poly = polys.at(cur_poly_index);
...
void PolyOps::printPolygons(const poly_list_t& polys) 
{
  for (uint i = 0; i < polys.size(); ++i) {
    poly p = polys.at(i);
    ROS_INFO("poly: %d S/T/CW: %d/%d/%d start: %d.%d.%d "
             "end: %d.%d.%d mp: %f,%f",
				   ...
				   ElementID from_id, ElementID to_id) 
{
  ...
  for (uint i = from_index; i < from_polys.size(); ++i) {
                ...
                const ElementID way0, const ElementID way1) 
{
  ...
  for (uint i = 0; i < polys_in_curr_lane_forward.size(); ++i) {
  ...
  for (uint i = 0; i < polys_in_left_lane_reverse.size(); ++i) {
                ...
                const ElementID way0) 
{
  ...
  for (uint i = 0; i < all_polys_in_curr_lane.size(); ++i) {
